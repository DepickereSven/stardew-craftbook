...
// each item the player owns anywhere, what category each of those items is in
// (so category ingredients like "any milk" can be resolved), and which recipes
// have been learned.
...
func addItems(snap *Snapshot, items []itemXML) {
	for _, it := range items {
		...
		// Saves may store either a bare id ("388") or a qualified one
		// ("(O)388"); the datasets use bare object ids throughout.
